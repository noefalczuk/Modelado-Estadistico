...
#Separación por paises

all_data_train <- titles_train
all_data_test <- titles_test

# 1. Filtrar las filas con NA o "[]"
all_data_train <- all_data_train %>%
  filter(!is.na(production_countries), production_countries != "[]")

# 2. Limpiar los caracteres [ ] '
all_data_train <- all_data_train %>%
  mutate(production_countries = str_replace_all(production_countries, "\\[|\\]|'", ""))

# 3. Separar en múltiples filas por cada país
all_data_train <- all_data_train %>%
  separate_rows(production_countries, sep = ",\\s*") %>%
  mutate(production_countries = str_trim(production_countries))

sum(all_data_train$production_countries == "[]", na.rm = TRUE)
#Vemos cantidad de peliculas sin pais de prodcción
cant <- sum(is.na(all_data_test$production_countries)) 
# Como se tiene aprox un 4% de instancias sin production_countries, vamos a elimiarlas del df
all_data_train <- all_data_train[all_data_train$production_countries != "[]", ]
...
all_data_train <- all_data_train %>% 
  mutate(
    country    = factor(production_countries),
  ...
  drop_na(imdb_score) %>% 
  select(-c(age_certification, ...1, imdb_id, id, genres, production_countries))
...
all_data_test <- all_data_test %>% 
  ...
  select(-c(age_certification, ...1, imdb_id, genres, production_countries))
...
# Modelo 3: Hecho mediante model selection por busqueda brute force, codigo en el anexo
m3 <- lmer(imdb_score ~ seasons + ns(imdb_votes, 8) + ns(runtime, 8) + ns(runtime_c, 8) + ns(Year_c, 8) + Comedy + Action + (1 | country),
           data = train_set, REML = FALSE)
pred3 <- predict(m3, newdata = valid_set, allow.new.levels = TRUE)
rmse3 <- rmse(valid_set$imdb_score, pred3)

rmse_tbl <- tibble(
  Modelo = c("Lineal + spline", "Mixto", "Brute Force Mixto + Spline"),
...
final_model <- lmer(imdb_score ~ seasons + ns(imdb_votes, 8) + ns(runtime, 8) + ns(runtime_c, 8) + ns(Year_c, 8) + Comedy + Action + (1 | country),
                      data = all_data_train, REML = FALSE)

best_preds <- predict(final_model, newdata = all_data_test, allow.new.levels = TRUE)
predicciones_promedio <- all_data_test %>%
  group_by(id) %>%
  summarise(pred_promedio = mean(best_preds, na.rm = TRUE))

write_csv(tibble(imdb_score_pred = preds), "predicciones.csv")
cat('Archivo predicciones.csv generado.\n')


# Anexo, model selection por brute force
df_spline <- 8

combis <- expand.grid(
  seasons   = c(TRUE, FALSE),
  imdb_votes= c(TRUE, FALSE),
  runtime   = c(TRUE, FALSE),
...
rmse_cv <- function(formula_obj, random_flag) {
  tryCatch({
    mod <- if (random_flag) {
      lme4::lmer(formula_obj, data = train_set, REML = FALSE)
    } else {
      stats::lm(formula_obj, data = train_set)
    }
    pred <- predict(mod, newdata = valid_set, allow.new.levels = TRUE)
    sqrt(mean((valid_set$imdb_score - pred)^2, na.rm = TRUE))
  },
